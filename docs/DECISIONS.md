...
- **Residual risk — amended after implementation, and it is worse than I estimated.** With the chain actually configured and every parameter summed, the measured-on-paper result is **1.9 µs typical but 2.6 µs on all-worst-case silicon**, against the 2 µs withstand. My original "~2.0 µs" understated it. So: *typical parts pass; worst-case parts do not.* The dominant term is the soft turn-off itself (1.15–1.43 µs to discharge the gate at the family-maximum 699 mA) — and that slowness is deliberate, because a hard turn-off into a short produces the overvoltage spike soft-off exists to prevent. This is not a value that can simply be tightened.
- **One physical factor works in our favour and must be measured, not assumed:** the module's t_SC is specified at **V_DD = 750 V**, while this design's maximum bus is 450 V and its nominal is 250 V. Short-circuit current, and therefore the rate of die heating, fall steeply with bus voltage, so the true withstand at our operating point is longer than the datasheet figure. That is a sound argument but an unquantified one — it is a reason to expect the validation to pass, not a substitute for running it.
- **Consequences, recorded rather than rounded away:** the exposure is a hard external short (phase-to-phase or phase-to-ground) on worst-case silicon at maximum junction temperature. The *internal* shoot-through case — the more likely one — is prevented upstream by the overlap eliminator, which is firmware-independent. **The double-pulse short-circuit validation is a gate on production release.** If it fails: (a) characterise a faster soft-off and accept the turn-off overvoltage it produces, which requires double-pulse measurement of the resulting spike against the 1200 V rating, or (b) reduce the peak current rating, which lengthens the withstand, or (c) reopen D017. None of these is a value tweak.
